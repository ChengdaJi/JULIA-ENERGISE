function GML_large(ancillary_type, T, BN, SN,
    ...
    # current_time=1
        ...
        price = price_traj_det(current_time, ancillary_type,
            price_raw, delta_rt_raw, T, pred_length);
        # println(size(price.lambda_da))
        # println(size(price.lambda_rt))
        # println(size(price.alpha_rt))

        pd = pd_traj_large_det(current_time, pd_raw, pd_noise, T, pred_length, base,multiplier);
        # println(size(pd.rt))
        # println(size(pd.da))
        ...
        global data = (pd_da = (pd.da),pd_rt = (pd.rt), pg_da=(pg.da), pg_rt=(pg.rt),
        lambda_da=(price.lambda_da),lambda_rt=(price.lambda_rt),
        alpha_rt=(price.alpha_rt))
    end
    return data
end
