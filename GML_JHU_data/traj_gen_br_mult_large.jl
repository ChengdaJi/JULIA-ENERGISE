...
function price_traj_det(t, ancillary_type, price_raw, delta_rt_raw, T, Pred_length)
# this function generates the price trajectory
    lambda_rt=price_raw.LMP_RT[t+1:t+T]
    lambda_da=price_raw.LMP_DA[t+1:t+T]

    if ancillary_type == "without"
        alpha_rt=zeros(1,T);
    elseif ancillary_type == "10min"
        alpha_rt=price_raw.RSRV_10[t+1:t+T];
    elseif ancillary_type == "30min"
        alpha_rt=price_raw.RSRV_30[t+1:t+T];
    end
    price = (lambda_rt=(lambda_rt), lambda_da=(lambda_da),
        alpha_rt=(alpha_rt))
    return price;
end
...
function pg_traj_large_det(t, pg_raw, pg_noise, solar_error_max, p_rate, T,
    ...
    rt = rt_raw[:, t+1:t+288]
    da = da_raw[:, t+1:t+288]
    pg=(rt=(rt/base.MVA), da=(da/base.MVA));
    return pg
end
...
function pg_traj_large_emergency(t, pg_raw, pg_noise, solar_error_max, p_rate, T,
    ...
    rt = rt_raw[:, t+1:t+288]
    da = da_raw[:, t+1:t+288]
    rt_5_emergency = p_rate*pg_raw.pg_rt[5,187:t+288]*(1-0.25)
    rt[5,186:t+287] = rt_5_emergency;
    pg=(rt=(rt/base.MVA), da=(da/base.MVA));
    return pg
end
