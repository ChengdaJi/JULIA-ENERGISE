...
function optimal_NYISO(SN, t, obj, ancillary_type, baseMVA,
    ...
    for bus=1:NoBus
        ...
        @constraint(m, Qf_min[bus,1]<=Q_bus_rt[bus,1]);
        @constraint(m, Q_bus_rt[bus,1]<=Qf_max[bus,1]);
        # @constraint(m, -100<=Q_bus_rt[bus,1]);
        # @constraint(m, Q_bus_rt[bus,1]<=100);
        ...
        # @constraint(m,
        # [sqrt(10)*(positive_scalar(
        # icdf*sqrt(pd.sigma[bus,1]+pg.sigma[bus,1])
        # +pg.mu[bus,1])+R_max[bus,1]-Pd[bus,1]),
        # P_bus_rt[bus,1], Q_bus_rt[bus,1]] in SecondOrderCone())
        # @constraint(m,
        # [sqrt(1+0.05^2)*(Pd[bus,1]+R_max[bus,1]-
        # positive_scalar(
        # icdf*sqrt(pd.sigma[bus,1]+pg.sigma[bus,1]))),
        # P_bus_rt[bus,1], Q_bus_rt[bus,1]] in SecondOrderCone())
    ...
    for scenario = 1:SN
        for t=1:T-1
            for bus=1:NoBus
                ...
                @constraint(m, Qf_min[bus,t+1] <= Q_bus[scenario,bus,t]);
                @constraint(m, Q_bus[scenario,bus,t]<= Qf_max[bus,t+1]);
                ...
                # @constraint(m, [sqrt(1+0.05^2)*(positive_scalar(
                # icdf*sqrt(pd.sigma[bus,t+1]+pg.sigma[bus,t+1])+pg.mu[bus,t+1])
                # +R_max[bus,t+1]-Pd[bus,t+1]),
                # P_bus[scenario,bus,t], Q_bus[scenario,bus,t]] in SecondOrderCone())
    ...
    #         2*(r[branch]*P_br_rt_o[branch,1]+x[branch]*Q_br_rt_o[branch,1]))
    # end
    # for t=1:287
    #     for branch =15:16
    #         fbus = Int(branch_struct.fbus[branch]);
