...
function optimal_stoach_scenario_large(current_time, obj, feedback, pd, pg, price, ancillary_type, base, network)
    println("===== GML - Optimization with Peak Shaving")
    ...
    ############# Peak Shaving #################
    @constraint(m, P_bus_rt[1,1] <= Scp[1,1]);
    ############################################
    ...
    for scenario = 1:SN
        for t=1:T-1
            ...
            ############# Peak Shaving #################
            @constraint(m, P_bus[scenario, 1, t] <= Scp[1,1]);
            ############################################
        end
    end
    ...
    if ancillary_type == "10min" || ancillary_type == "30min"
        @objective(m, Min,
            fn_cost_RHC_anc_Peak(delta_t,P_bus_rt,P_bus,Pg_rt,Pg,P_rsrv_rt,Scp,
...
function optimal_stoach_scenario_large_con(current_time, obj, feedback, pd, pg, price, ancillary_type, base, network)
    println("===== GML - Optimization with N-1 and Peak Shaving")
...
#     return Final_cost
# end
function fn_cost_RHC_anc_Peak(delta_t,P_bus_rt,P_bus,Pg_rt,Pg,P_rsrv_rt, Scp,
    P_rsrv,price,pg,pd,beta,SN,obj,base,gen)
...
# function fn_cost_RHC_rt_Scp(delta_t,P_bus_rt,P_bus,Pg_rt,Pg,price,Scp
#     pg,pd,beta,SN,obj,base,gen)
#     T=obj.T;
#     icdf = obj.icdf;
#     gen_list = [1];
