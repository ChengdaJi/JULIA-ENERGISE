function GML_large_emergency(ancillary_type, T, BN, SN,
    ...
    # current_time=1
        ...
        if current_time == T_emergency
            B_feedback_temp = (1-pcent_loss)*B_feedback[5,1];
            B_feedback[5,1] = B_feedback_temp;
            feedback = (B_feedback=(B_feedback), P_rsrv_feedback=(P_rsrv_feedback));
            # println(feedback.B_feedback)
            pg = pg_traj_large_emergency(current_time, pg_raw, pg_noise,
                solar_error_max, p_rate, T, pred_length,  base, multiplier, pcent_loss);
            obj = GML_Sys_Ava_large_emergency(T, BN, SN, pd, ancillary_type, icdf,
                B_cap, base, network,pcent_loss);
            val_opt = optimal_stoach_scenario_large_con_relax(current_time, obj, feedback, pd, pg,
                price, ancillary_type, base, network, T_emergency)

        elseif current_time in T_emergency+1:T_emergency+tau+k-1
            feedback = (B_feedback=(B_feedback), P_rsrv_feedback=(P_rsrv_feedback));
            # println(feedback.B_feedback)
            pg = pg_traj_large_emergency(current_time, pg_raw, pg_noise,
                solar_error_max, p_rate, T, pred_length,  base, multiplier, pcent_loss);
            ...
            val_opt = optimal_stoach_scenario_large_con_relax(current_time, obj, feedback, pd, pg,
                price, ancillary_type, base, network, T_emergency)
        elseif current_time >= T_emergency+tau+k
        ...
        elseif current_time < T_emergency
