...
for current_time=1:1
    if mod(current_time,12) == 0
        ...
        T=1+Pred_length+(12-mod(current_time,12));
    end
    ...
    if current_time ==1
        ...
        global B_feedback=repeat(repeat(reshape([0.0910,0.0942,0.1434,0.1402,0.1382,0.2144,0.1192,0.1813,
            0.1355,0.0664,0.1182,0.0580],12,1)), ceil(Int, BN/3), 1);
    end
    ...
    pg = pg_traj(current_time, pg_raw, pg_noise, solar_error_max, p_rate, BN, T, Pred_length);
    ...
    write_output_out(val_opt, current_time, F)
    println("=================================================")
end
