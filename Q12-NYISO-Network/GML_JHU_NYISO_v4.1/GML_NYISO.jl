function GML_solver(date, current_time, B_inp, raw_data)
    ...
    # price = price_traj_det(date, current_time, ancillary_type, price_raw, T);
...
function GML_Sys_Ava_NYISO(T, pd, ancillary_type, B_cap, icdf, shunt_struct,
    ...
    B_max = (reshape(shunt_struct.frac_P*B_cap, NoShunt, 1)*ones(1, T))*B_cap/(B_cap-1.358);
...
function optimal_NYISO(SN, t, obj, ancillary_type, baseMVA,
    ...
    @variable(m, Qg[1:SN, 1:NoShunt, 1:T-1]); # the real power output

    @variable(m, B[1:SN, 1:NoShunt, 1:T-1]); # the storage
    @variable(m, R[1:SN, 1:NoShunt, 1:T-1]);# the charge/discharge rate
    @variable(m, P_shunt[1:SN, 1:NoShunt, 1:T-1])
    @variable(m, Q_shunt[1:SN, 1:NoShunt, 1:T-1])
...
function fn_cost_RHC_rt(delta_t, P_gen, P_gen_rt, Pg_rt, Pg, price,
    ...
    Final_cost = (Cost_P_gen_sum_ct+Cost_P_gen_sum_scenario
        +Cost_Pg_ct_diff+Cost_Pg_scenario_diff+Cost_peakshaving)
