function GML_solver(date, current_time, initial_time, raw_data)
    ...
    reference_points = reference_point_layer3(sum(Pd_bus), Pd_bus, Qd_bus);
    ...
    val_rp = reference_point_layer3(sum(Pd_bus), Pd_bus_real, Qd_bus_real);
    # val_rp = reference_point(sum(Pd_bus), Pd_bus_real, Qd_bus_real);


    file_type = string("Q=", p_rate,"_")
    # filename_p_gen = string(file_type,"P_gen.csv")
    # write_star_out(filename_p_gen, val_opt.P_gen, val_rp.P_gen)
    #
    #
    # filename_q_gen = string(file_type,"Q_gen.csv")
    # write_star_out(filename_q_gen, val_opt.Q_gen, val_rp.Q_gen)
    #
    #
    # filename_v = string(file_type,"v.csv")
    # write_star_out(filename_v, val_opt.v, val_rp.v_magnitude)
    #
    #
    # filename_theta = string(file_type,"theta.csv")
    # write_star_out(filename_theta, val_opt.theta, val_rp.v_polar)

    filename_p_gen = string(file_type,"P_gen.csv")
    write_star_out2(filename_p_gen, val_opt.P_gen, val_rp.P_gen, reference_points.P_gen)


    filename_q_gen = string(file_type,"Q_gen.csv")
    write_star_out2(filename_q_gen, val_opt.Q_gen, val_rp.Q_gen, reference_points.Q_gen)


    filename_v = string(file_type,"v.csv")
    write_star_out2(filename_v, val_opt.v, val_rp.v_magnitude, reference_points.v_magnitude)


    filename_theta = string(file_type,"theta.csv")
    write_star_out2(filename_theta, val_opt.theta, val_rp.v_polar, reference_points.v_polar)
...
function GML_Sys_Ava_NYISO(T, pd, ancillary_type, B_cap, icdf, shunt_struct,
    ...
    if B_cap > 0
        B_max = reshape(shunt_struct.frac_P*B_cap, NoShunt, 1)*ones(1, T);
...
function optimal_NYISO(SN, t, obj, ancillary_type, baseMVA,
    ...
    P_Percent = [0.181935169, 0.002855468, 0.001789426, 0.004736269, 0.030496394,
        ...
        @variable(m, -Qd[shunt,1]+pg.sg_max[shunt, 1]+270>= Q_shunt_rt[shunt = 1:NoShunt, 1]
            >=-Qd[shunt,1]-pg.sg_max[shunt, 1]-270)

        # bus level
        @variable(m, 1000 >= P_bus_rt[1:NoBus, 1]  >= -1000)
        @variable(m, 1000 >= Q_bus_rt[1:NoBus, 1] >= -1000)
        @variable(m, pi/2 >= theta_rt[1:NoBus, 1] >= -pi/2)
...
function write_star_out2(filename, GML_data, Real_data, star_data)
    # println(P_rsrv_feedback)
    Data_length = length(GML_data);
    # println(size(reshape(GML_data, Data_length, 1)));
    # println(size(reshape(Real_data, Data_length, 1)));
    ...
    #     reshape(Real_data, Data_length, 1)))
    CSV.write(filename, DataFrame(hcat(reshape(GML_data, Data_length, 1),
        reshape(Real_data, Data_length, 1), reshape(star_data, Data_length, 1)),
        [:GML, :Layer3, :Layer1]));
end
...
function reference_point(Pd_sum, Pd_bus, Qd_bus)
    ...
    for (gen_name, gen_data) in network_data["gen"]
        ...
        if gen_id != 1
            ...
            gen_data["pmax"] = Pd_sum*P_Percent[gen_id];
            gen_data["pmin"] = Pd_sum*P_Percent[gen_id];
            println(gen_id)
            println(gen_data["pg"])
        end
    end
    ...
    power_flow_result = run_opf(network_data, ACPPowerModel, Ipopt.Optimizer);
...
function reference_point_layer3(Pd_sum, Pd_bus, Qd_bus)
