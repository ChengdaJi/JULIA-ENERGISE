...
function optimal_NYISO(SN, t, obj, ancillary_type, baseMVA,
    ...
    # Shunt level
    @variable(m, 0<=Pg_rt[shunt=1:NoShunt, 1]<=pg.mu[shunt,1])
    @variable(m, pg.sg_max[shunt, 1] >= Qg_rt[shunt=1:NoShunt, 1] >= -pg.sg_max[shunt, 1])
    # @variable(m, temp[1:NoShunt, 1])
    @variable(m,  B_rt[shunt = 1:NoShunt, 1] == 0)
    @variable(m, R_rt[shunt = 1:NoShunt, 1] == 0)
    ...
    for shunt=1:NoShunt
        ...
        if shunt_struct.type[shunt, 1]==1
            # solar
            @constraint(m, Pg_rt[shunt,1]==pg.mu[shunt,1])
    ...
    @variable(m,
        R[1:SN, shunt=1:NoShunt, t=1:T-1] == 0);# the charge/discharge rate

    @variable(m, Pd[shunt,t+1] >=
        P_shunt[1:SN, shunt=1:NoShunt, t=1:T-1] >=
        Pd[shunt,t+1]-pg.mu[shunt, t+1])
    ...
    for scenario = 1:SN
        ...
        for t=1:T-1
            ...
            for shunt=1:NoShunt
                ...
                # end
                if shunt_struct.type[shunt, 1]==1
                    @constraint(m, Pg[scenario, shunt,t]==pg.mu[shunt, t+1])
                    #     positive_scalar(
...
function write_output_out_2(val_opt, P_rsrv_feed, val_rp, price, Pd_real, filename)
    ...
    RT_data_feeder=hcat(cost, time, Pg, B, R, Pd, P0, P_rsrv, P_cul, status)
    CSV.write(filename, DataFrame(RT_data_feeder,
        [:cost, :time, :Pg, :B, :R, :Pd_net, :P0, :P_rsrv, :P_cul, :status]));
    println("    ---- Finish writting files! ")
end
