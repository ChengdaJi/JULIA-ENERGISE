function GML_solver(date, current_time, B_inp,
        raw_data, solar_percent, battery_capacity_percent, Q_solar_flag)
...
function optimal_NYISO(SN, t, obj, ancillary_type, baseMVA,
    ...
    # @variable(m, Pg_rt[shunt=1:NoShunt, 1] == 0)
    if Q_solar_flag == 0
        @variable(m, Qg_rt[shunt=1:NoShunt, 1] == 0)
    else
        @variable(m, pg.sg_max[shunt, 1]>=Qg_rt[shunt=1:NoShunt, 1]
            >= -pg.sg_max[shunt, 1])
    end
    ...
    for bus=1:NoBus
        ...
        # if bus == 70
        #     @constraint(m,
        #         reference_points.v_star[bus,1] == v_rt[bus,1]);
        # else
        ...
        if bus_struct.type[bus]==1 # non-generator bus
            ...
            @constraint(m, reference_points.v_star[bus,1] == v_rt[bus,1]);
        ...
        for t=1:T-1
            ...
            for bus=1:NoBus
                ...
                if bus_struct.type[bus] == 1 # non-generator bus
                    ...
                    @constraint(m, reference_points.v_star[bus,1] == v[scenario,bus,t]);
                    gen_id = findall(bus_id ->bus_id == bus, gen_struct.bus[:,1]);
            ...
            for Gen=2:NoGen
                ...
                @constraint(m,
                    Q_gen[scenario,Gen,t]<=10*gen_struct.Qmax[Gen]/baseMVA)
                @constraint(m,
                    Q_gen[scenario,Gen,t]>=-10*gen_struct.Qmax[Gen]/baseMVA)
            end
            @constraint(m,
