function price_traj(t, date, ancillary_type, price_raw, delta_rt_raw, T, Pred_length)
# this function generates the price trajectory
    ...
    ## read price noise
    price_noise_file = string(
        "GML_data/price-solar-demand/price_noise/price_noise_",date,"_time",t,".mat")
    price_noise = matread(price_noise_file)["price_noise"];
    println(size(price_noise))
    # println(zero_flag)
    ...
    lambda_pd=reshape(price_raw.LMP_RT[t+1:t+Pred_length],1,Pred_length
        )+reshape(price_noise[1,1:Pred_length],1,Pred_length)
    ...
    # println(size(lambda_da))
    lambda_offline=reshape(lambda_da, 1, length(lambda_da));
...
function pd_traj(date, t, pd_raw, pd_noise, T, NoShunt, Pred_length, baseMVA)
    ...
    raw_pu_traj[1:NoShunt, :]=pd_raw.pd_rt[1:NoShunt, t+1:t+T]/baseMVA;
    raw_pu_qd_traj[1:NoShunt, :]=pd_raw.qd_rt[1:NoShunt, t+1:t+T]/baseMVA;
    raw_pu_da[1:NoShunt, :]=pd_raw.pd_da[1:NoShunt,t+1:t+T]/baseMVA;
    raw_pu_qd_da[1:NoShunt, :]=pd_raw.qd_da[1:NoShunt,t+1:t+T]/baseMVA;
    ...
    qd_pred = zeros(NoShunt, Pred_length);
    for shunt=1:NoShunt
        for predict_time = 1:Pred_length
            pred[shunt, predict_time] =
                raw_pu_traj[shunt,Pred_length]*(1+pd_noise[shunt,predict_time]);
            qd_pred[shunt, predict_time] =
                raw_pu_qd_traj[shunt,Pred_length]*(1+pd_noise[shunt,predict_time]);
        end
    end
...
function pg_traj(date, t, pg_raw, pg_noise, solar_error_max, p_rate, T, Pred_length,
    ...
    if solar_error_max >=0
        ...
        for shunt=1:NoShunt
            for predict_time = 1:Pred_length
                temp_solar = rt_raw[shunt,t+Pred_length]*(1+pg_noise[shunt,predict_time]);
                if temp_solar >=0
                    pred[shunt, predict_time]=temp_solar;
                end
            end
        end
...
###############################
function read_NY_demand_data(date,shunt)
    # P=shunt.P;
    NoShunt = length(shunt.P);
    demand_file_name =
    	string("GML_data/price-solar-demand/rt_demand_solar/demand_",date,".mat");
    demand_unit=matread(demand_file_name)["real_time"];
    #################################
    # 576*240
    ...
    da_demand_file_name =
    	string("GML_data/price-solar-demand/rt_demand_solar/demand_da_",date,".mat");
    demand_da_unit=matread(da_demand_file_name)["day_ahead"]
...
###################################
# raw data
function read_NY_solar_data(date,pd_raw)
    # P=shunt.P;
    NoShunt = length(pd_raw.pd_rt[:,1]);
    solar_file_name =
    	string("GML_data/price-solar-demand/rt_demand_solar/solar_",date,".mat");
    solar_unit=matread(solar_file_name)["real_time"]
    ...
    #
    solar_da_file_name =
    	string("GML_data/price-solar-demand/rt_demand_solar/solar_",date,"_day_ahead.mat");
    solar_da_unit=matread(solar_da_file_name)["day_ahead"]
...
function pre_process(date)
    ...
    pd_raw = read_NY_demand_data(date, shunt_struct)
    ...
    #
    # # ###########################################################################
    pg_raw = read_NY_solar_data(date, pd_raw)
    # # #
