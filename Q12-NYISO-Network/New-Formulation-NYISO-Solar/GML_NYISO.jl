    ...
    for bus=1:NoBus
        ...
        if ~isempty(bus_shunt_list)
            ...
            Qd_bus_real[bus,1] =
                ...
                for shunt in bus_shunt_list);
            # Qd_bus_real[bus,1] = sum(val_opt.Q_shunt[Int(shunt),1]
            # for shunt in bus_shunt_list)
...
function optimal_NYISO(SN, t, obj, ancillary_type, baseMVA,
    ...
    P_Percent = [0.181935169, 0.002855468, 0.001789426, 0.004736269, 0.030496394,
        ...
        # Peak-shaving
        @variable(m, 10 >=peak_withdraw[1, 1] >= 0)
        ...
        @variable(m, 0.05*pg.mu[shunt,1] >=
            Qg_rt[shunt=1:NoShunt, 1]>= -0.05*pg.mu[shunt,1] )
        # @variable(m, pg.sg_max[shunt, 1] >= lg[shunt=1:NoShunt, 1]
        #     >= pg.sg_max[shunt, 1]/2)
        ...
        # @variable(m, -10<= P_shunt_rt[shunt = 1:NoShunt, 1]
        #     <= 10)

        # @variable(m, -Qd[shunt,1]+pg.sg_max[shunt, 1]+2.70>= Q_shunt_rt[shunt = 1:NoShunt, 1]
        #     >=-Qd[shunt,1]-pg.sg_max[shunt, 1]-2.70)
        @variable(m, 10 >= Q_shunt_rt[shunt = 1:NoShunt, 1] >=-10)
        ...
        for shunt=1:NoShunt
            ...
            if shunt_struct.type[shunt, 1]==1 # load shunt
                # solar
                # @constraint(m, [lg[shunt, 1], Pg_rt[shunt,1], Qg_rt[shunt,1]]
                #     in SecondOrderCone());
                ...
                # @constraint(m,-Qd[shunt,1]-sqrt(pg.sg_max[shunt, 1]^2-pg.mu[shunt,1]^2)
                #     <=Q_shunt_rt[shunt,1])
                # @constraint(m, Q_shunt_rt[shunt,1]<=
                #     -Qd[shunt,1]+sqrt(pg.sg_max[shunt, 1]^2-pg.mu[shunt,1]^2));
                # @constraint(m,-Qd[shunt,1] == Q_shunt_rt[shunt,1])
        ...
        @variable(m, theta[1:SN, 1:NoBus, 1:T-1] == 0)
