...
solar_percent = 0.5;
battery_capacity_percent = 0.5;

val_opt = GML_solver(date, current_time, B_inp, raw_data, solar_percent, battery_capacity_percent)
#
println("battery discharge [p.u.] size 3 feeder * 288 time slots (1 hr)")
println(val_opt.R_FOL)
println("Solar usage [p.u.] size 3 feeder * 288 time slots (1 hr)")
println(val_opt.Pg_FOL)
println("Net real power (withdraw) [p.u.] size 3 feeder * 288 time slots (1 hr)")
println(val_opt.P_FOL)
println("Net reactive power (withdraw) [p.u.] size 3 feeder * 288 time slots (1 hr)")
println(val_opt.Q_FOL)
println("voltage magnitude [p.u.] size 3 feeder * 288 time slots (1 hr)")
println(val_opt.v_FOL)
println("battery discharge [p.u.] size all feeders * 288 time slots (1 hr)")
println(val_opt.R_feeder)
println("Solar usage [p.u.] size all feeders * 288 time slots (1 hr)")
println(val_opt.Pg_feeder)
println("Net real power (withdraw) [p.u.] size all feeders * 288 time slots (1 hr)")
println(val_opt.P_feeder)
println("Net reactive power (withdraw) [p.u.] size all feeders * 288 time slots (1 hr)")
println(val_opt.Q_feeder)
println("voltage magnitude [p.u.] size all feeders * 288 time slots (1 hr)")
println(val_opt.v_feeder)
#
