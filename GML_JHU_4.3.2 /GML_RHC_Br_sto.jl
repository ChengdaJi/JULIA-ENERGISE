...
function optimal_stoach_scenario_large(current_time, obj, feedback, pd, pg, price, ancillary_type, base, network)
    ...
    if ancillary_type == "10min" || ancillary_type == "30min"
       ...
       # RT
        ...
        @variable(m, Surplus_rt)
        @constraint(m, 0 <= Surplus_rt)
       ...
       # @constraint(m, P_rsrv_rt==3)
        ...
        @constraint(m, B_rsrv_rt <= sum(B_rt)+Surplus_rt)
        ...
        @variable(m, Surplus[1:SN, 1:T-1])

        # @constraint(m, P_rsrv==3)

        for scenario=1:SN
            for t_real=current_time+1:current_time+T-1
                @constraint(m, 0 <= Surplus[scenario, t_real-current_time])
                ...
                @constraint(m, B_rsrv[scenario, t_real-current_time]
                     ...
                     Surplus[scenario, t_real-current_time])
                # @constraint(m, B_rsrv[scenario, t_real-current_time]
                #      <= 4)
           end
       end
    ...
    if ancillary_type == "10min" || ancillary_type == "30min"
        @objective(m, Min,
            fn_cost_RHC_anc_surplus(delta_t,P_bus_rt,P_bus,Pg_rt,Pg,P_rsrv_rt,
                P_rsrv,price,pg,pd,beta,SN,obj,base,gen, Surplus_rt,Surplus))
    # else
    #     @objective(m, Min,
    ...
    #             pg,pd,beta,SN,obj,base,gen))
     ...
     ############
     Surplus_rt_o = JuMP.value(Surplus_rt);
     println(string("surplus ", Surplus_rt_o))
     # println
     if sum(Pg_rt_o)<=sum(pg.mu_ct)
     ############
         Cost_real = delta_t*(P_bus_rt_o[1,1]*price.lambda_ct
             +beta*(sum(Pg_rt_o)-sum(pg.mu_ct)))*base.MVA-
             delta_t*P_rsrv_rt_o*price.alpha_ct+Surplus_rt_o*2000;
     else
         Cost_real = delta_t*(P_bus_rt_o[1,1]*price.lambda_ct
...
#
#     # =======
#     P_gen_sum_scenario = reshape(P_bus[1, 1, :],T-1,1)
#
#     lambda_scenario = reshape(price.probability[1]/sum_prob*
#         price.lambda_scenario[1,2:T],1,T-1)
#
...
#     Revenue_P_rsrv_scenario = price.probability[1]/sum_prob*delta_t*
#         reshape(P_rsrv[1, :], 1, T-1)*
...
#     if SN>1
#         for scenario=2:SN
#             P_gen_sum_scenario = reshape(P_bus[scenario, 1, :],T-1,1)
#
...
#
#             Revenue_P_rsrv_scenario = Revenue_P_rsrv_scenario+
...
#
#     Final_cost = ((Cost_P_gen_sum_ct+Cost_P_gen_sum_scenario[1,1]
...
function fn_cost_RHC_anc_surplus(delta_t,P_bus_rt,P_bus,Pg_rt,Pg,P_rsrv_rt,
    P_rsrv,price,pg,pd,beta,SN,obj,base,gen, Surplus_rt,Surplus)
    ...
    Cost_surplus = 2000*(Surplus_rt+sum(Surplus));
    ...
    Final_cost = ((Cost_P_gen_sum_ct+Cost_P_gen_sum_scenario
        +Cost_Pg_ct_diff+Cost_Pg_scenario_diff)*base.MVA
        -Revenue_P_rsrv_ct-Revenue_P_rsrv_scenario[1,1])+Cost_surplus
    return Final_cost
end
...
function write_output_out(val_opt, P_rsrv_feed, filename)
    ...
    RT_data_feeder=hcat(cost, time, Pg, B, R, P0, P_rsrv, B_rsrv, P_cul, status)
    CSV.write(filename, DataFrame(RT_data_feeder,
        [:cost, :time, :Pg, :B, :R, :P0, :P_rsrv, :B_rsrv, :P_cul, :status]));
