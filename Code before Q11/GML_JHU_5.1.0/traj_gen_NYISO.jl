...
function pd_traj_pu_det(t, pd_raw, T, NoBus, baseMVA)
    # this function generates the demand trajectory
    traj=zeros(NoBus, 288)
    da = zeros(NoBus, 288)
    for bus=1:NoBus
        traj[bus, :]=pd_raw.pd_rt[bus, t+1:t+T]/baseMVA;
...
function pg_traj_pu_det(t, pg_raw, p_rate, T, NoBus, baseMVA);
    #########
    parameter_pd_pg=1.0417852280636593;
    # sum(pd_raw)/sum(pg_raw) for first 288 slots
    #########
